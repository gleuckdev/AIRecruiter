...
class JobToken(db.Model):
    """
    JobToken model for grouping similar job postings
    """
    __tablename__ = 'job_tokens'
    
    id = db.Column(db.Integer, primary_key=True)
    token_hash = db.Column(db.String(64), unique=True, index=True)
    base_title = db.Column(db.String(255))
    base_location = db.Column(db.String(255))
    description_vector = db.Column(db.Text)  # JSON string of the embedding vector
    job_count = db.Column(db.Integer, default=1)
    created_at = db.Column(db.DateTime, default=datetime.utcnow)
    jobs = db.relationship('Job', backref='token', lazy=True)
...
class Job(db.Model):
    ...
    token_id = db.Column(db.Integer, db.ForeignKey('job_tokens.id'))
    ...
    def is_expiring_soon(self, threshold_days=7):
        """
        Check if the job is expiring soon, within threshold_days
        
        Args:
            threshold_days: Number of days to consider as "expiring soon"
            
        Returns:
            bool: True if the job is active and expiring within threshold_days
        """
        if self.status != 'active':
            return False
            
        time_until_expiry = self.expires_at - datetime.utcnow()
        return time_until_expiry.days <= threshold_days and time_until_expiry.days >= 0
        
    def renew(self, days=60):
        """
        Renew the job posting for the specified number of days
        
        Args:
            days: Number of days to extend the job posting
...
class Candidate(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    name = db.Column(db.String(100), nullable=False)
    email = db.Column(db.String(120), nullable=True)  # Allow null/empty emails
