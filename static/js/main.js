        ...
        try {
            ...
            if (response.ok) {
                ...
                if (data.code === 'permission_denied') {
                    let errorMessage = `<strong>Permission Denied:</strong> ${data.details || 'You do not have permission to perform this action.'}<br>`;
                    errorMessage += `Required role: ${data.required_role || 'Senior Recruiter or Administrator'}<br>`;
                    errorMessage += `Your current role: ${data.current_role || 'Recruiter'}<br>`;
                    errorMessage += `<br>Please contact your administrator to request higher permissions.`;
                    
                    showMessage(messageContainer, 'error', errorMessage, true);
                } else {
            ...
            // Check if this is a permission error from HTML response
            if (error.message && error.message.includes('Unexpected token')) {
                showMessage(messageContainer, 'error', `<strong>Permission Error:</strong> You don't have permission to create jobs.<br><br>This action requires the 'jobs:create' permission which is available to Senior Recruiters and Administrators.<br><br>Please contact your administrator to request this permission.`, true);
            } else {
